...
def run(
    ...
    if not training:
        ...
        # Calculate FPS
        total_processing_time = sum(t)
        fps = 1000 / total_processing_time
        formatted_fps = round(fps, 3)
        LOGGER.info(f'FPS: {formatted_fps}')
    # Plots
    if plots:
...
def parse_opt():
    ...
    parser.add_argument("--data", type=str, default="/root/autodl-tmp/RAFDB_YOLO-detection/data.yaml", help="dataset.yaml path")
    parser.add_argument("--weights", nargs="+", type=str, default="/root/autodl-tmp/yolov5pt/v5-RAFDB-Detection-best.pt", help="model path(s)")
